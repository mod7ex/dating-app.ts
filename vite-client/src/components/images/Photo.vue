...
const imgContainer = ref<HTMLElement>(null);
...
onMounted(() => {
  ...
  img.value.onload = function () {
    ...
    setTimeout(() => {
      const percentage = img.value.width / props.containerWidth;

      console.log(percentage);

      imgContainer.value.style.flexGrow = `${percentage + 1}`;

      imgContainer.value.style.maxWidth = `${img.value.width * (percentage + 1.37)}px`;
    }, 1000);
  };
});
</script>
...
<template>
	<div class="photo" :ref="(el) => (imgContainer = el)" @mouseenter="mouseIn = true" @mouseleave="mouseIn = false">
